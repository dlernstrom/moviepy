...
class AudioClip(Clip):
        ...
        def generator():
            for i in range(nchunks):
                ...
                yield self.to_soundarray(
                    tt, nbytes=nbytes, quantize=quantize, fps=fps,
    ...
    def to_soundarray(self, tt=None, fps=None, quantize=False, nbytes=2,
        ...
        if tt is None:
            if self.duration > max_duration:
                return stacker(
                    self.iter_chunks(fps=fps, quantize=quantize, nbytes=2,
    ...
    def max_volume(self, stereo=False, chunksize=50000, progress_bar=False):
        ...
        for chunk in self.iter_chunks(chunksize=chunksize,
            ...
            maxi = np.maximum(maxi, abs(chunk).max(axis=0)) if stereo \
    ...
    def write_audiofile(self, filename, fps=44100, nbytes=2,
        ...
        return ffmpeg_audiowrite(
            self, filename, fps, nbytes, buffersize,
