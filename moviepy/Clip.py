...
from moviepy.decorators import apply_to_mask, apply_to_audio, \
    requires_duration, outplace, convert_to_seconds, use_clip_fps_by_default
...
class Clip(object):
    ...
    def get_frame(self, t):
        ...
        at time t or (mono or stereo) value for a sound clip
        """
    ...
    def subclip(self, t_start=0, t_end=None):
        ...
        if t_end is None and self.duration is not None:
            ...
            if self.duration is None:
                msg = ("Error: subclip with negative times (here %s)",
                       " can only be extracted from clips with a ``duration``")
                print ''.join(msg) % str((t_start, t_end))
            else:
