...
if FFMPEG_BINARY == 'ffmpeg-imageio':
    ...
    if not success:
        raise IOError(
            "%sThe path specified for the ffmpeg binary might be wrong" % \
            err.message)
...
if IMAGEMAGICK_BINARY == 'auto-detect':
    if os.name == 'nt':
        try:
            ...
            IMAGEMAGICK_BINARY = \
                wr.QueryValueEx(key, 'BinPath')[0] + r"\convert.exe"
            key.Close()
        except:
    ...
    if not success:
        raise IOError(
            "%s Path specified for the ImageMagick binary might be wrong" % \
            err.message)
